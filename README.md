...
| Note記事生成（MBTICODE／s4lv） | `docs/sop/sop_note_article.md` | `/note-article`・`/quality-guardrail` |
| junk_juice 記事（クライアント） | `docs/sop/sop_junk_juice.md` | `/junk-theme`・`/junk-article` |
...
├── .claude/commands/      ← 汎用スキル15本（投稿生成・記事生成・会議システム等）★唯一の正
├── docs/
