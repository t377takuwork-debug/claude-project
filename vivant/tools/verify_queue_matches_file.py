    ...
    while i < len(lines):
        ...
        if m and "URL候補" not in m.group(1):
            ...
            if j < len(lines) and SEP_RE.match(lines[j]):
                ...
                # あるときだけ自己リプライとして拾う（末尾の読者リプライログ等を巻き込まない）
                reply = ""
                end = k
                ...
                posts.append({"header": header, "body": body, "reply": reply, "type": type_label})
                i = end
                continue
        i += 1
    return posts
...
def main():
    ...
    for p in posts:
        ...
        targets[key] = {"body": p["body"], "reply": p["reply"], "type": p["type"], "header": p["header"]}
        order.append(key)
    ...
    for key in order:
        ...
        if s["body"].strip() != t["body"].strip():
            diffs.append("本文")
        if (s["reply"] or "").strip() != (t["reply"] or "").strip():
            diffs.append("リプライ")
