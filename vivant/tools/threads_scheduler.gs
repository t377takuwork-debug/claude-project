// vivant Threads scheduled poster + insight collector (Google Apps Script,
// bound to the queue spreadsheet)
//
// Ported from brands/mbticode/tools/threads_scheduler.gs (2026-08-02). This is
// the Phase1-4 subset plus a READ-ONLY GitHub sync (2026-08-02 addition) for
// weekly cloud-routine analysis. Unlike MBTICODE's full Phase5, there is NO
...
const GITHUB_SYNC_PATH = "vivant/tools/_synced_observation_data.json";
...
function syncDataToGitHub() {
  ...
  try {
    githubPutFile(GITHUB_SYNC_PATH, buildObservationPayload(),
      "Auto-sync vivant Threads observation data (" + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd") + ")");
...
function checkHealth() {
  ...
  if (problems.length > 0) {
    ...
    MailApp.sendEmail(Session.getEffectiveUser().getEmail(), "[vivant Threads] 監視アラート", body);
