# vtuber_log WordPress REST API connectivity test (ASCII-only source for PS5.1)
...
if (-not (Test-Path $authFile)) {
    ...
    Write-Host '{ "site": "https://vtuber-verse.com", "user": "YOUR-WP-USER", "appPassword": "xxxx xxxx xxxx xxxx xxxx xxxx" }'
    Write-Host "Get the app password in WP admin: Users > Profile > Application Passwords (name: vtuber-article)."
    exit 1
}
...
function Step-Media {
    ...
    $generated = $false
    if ($file -eq "") {
        $file = Join-Path $env:TEMP "vtuber-article-connect-test.png"
...
function Step-Post {
    ...
    $blocks = "<!-- wp:paragraph --><p>vtuber-article connectivity test. Safe to delete.</p><!-- /wp:paragraph -->" +
              "<!-- wp:image --><figure class=""wp-block-image size-large""><img src=""$($state.mediaUrl)"" alt=""vtuber-article connect test image"" width=""120""/></figure><!-- /wp:image -->"
    $body = @{ title = "vtuber-article connect test"; status = "draft"; content = $blocks } | ConvertTo-Json
    try {
        ...
        Write-Host "     check it in WP admin: Posts > Drafts > 'vtuber-article connect test'"
        return $true
    } catch {
