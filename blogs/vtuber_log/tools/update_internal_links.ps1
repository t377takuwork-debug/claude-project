# vtuber_log internal_links.md freshness checker (ASCII-only source for PS5.1)
# Compares live sitemap URLs (vtuber-verse.com) with rules\internal_links.md and reports the diff.
...
$indexUrl = "https://vtuber-verse.com/sitemap.xml"

# URLs that resolve HTTP 200 but are intentionally absent from the sitemap.
# None known for vtuber-verse.com as of 2026-07-12 (all categories are in the sitemap).
$ignoreStale = @()
...
Write-Output "=== vtuber_log internal links freshness check ==="
...
$live = @($live | ForEach-Object { $_.TrimEnd('/') } | Where-Object { $_ -ne 'https://vtuber-verse.com' } | Sort-Object -Unique)
if ($live.Count -eq 0) {
...
$known = @([regex]::Matches($linksRaw, 'https://vtuber-verse\.com[^\s\)\|<]*') | ForEach-Object { $_.Value.TrimEnd('/') } | Where-Object { $_ -ne 'https://vtuber-verse.com' } | Sort-Object -Unique)
