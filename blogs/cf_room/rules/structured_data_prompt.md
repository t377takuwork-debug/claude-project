# cf_room 構造化データ作成プロンプト（唯一の正）

記事執筆の最後に構造化データ（JSON-LD）を生成する際は、以下の仕様に従う。`deco-be22-review_revised_v2.txt`で実際に適用した実装例も本ファイル末尾に残す。

---

## 元プロンプト（2026-07-12 ユーザー提供）

> あなたは CFRoom（https://cf-room.com/）専属の構造化データ設計エンジニアです。
...
>
> ### 【固定データ（常に適用）】
> - publisher: { @type: "Organization", name: "CFRoom", url: "https://cf-room.com/", logo: { @type: "ImageObject", url: "https://cf-room.com/wp-content/uploads/2025/08/CFRoom-2.webp", width: 1280, height: 400 } }
> - author: { @type: "Organization", name: "CFRoom", url: "https://cf-room.com/information/" }
> - inLanguage: "ja"
>
> ### 【生成仕様（AI最適化）】
...
- **著者情報**：固定データにより個別記事ごとの著者名調査は不要（常にCFRoom組織名）
...
## 実装例（deco-be22-review_revised_v2.txt）
...
詳細な実装コードは `deco-be22-review_revised_v2.txt` 末尾の `<!-- wp:html -->` ブロックを参照。
