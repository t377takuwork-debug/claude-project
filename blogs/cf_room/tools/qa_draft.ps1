# cf_room draft QA script (ASCII-only source to avoid PS5.1 encoding issues)
...
Write-Output "=== cf_room qa_draft: $Path ==="

# 1. Block comment open/close balance
foreach ($tag in @("paragraph", "heading", "image", "table", "html", "list", "quote", "separator")) {
...
#    Skips /wp-content/ (images). Allowlist: site root and /information/ (fixed JSON-LD data).
...
if (Test-Path $linksFile) {
    ...
    $known = @([regex]::Matches($linksRaw, 'https://cf-room\.com[^\s\)\|<]*') | ForEach-Object { $_.Value.TrimEnd('/') } | Sort-Object -Unique)
    $allow = @('https://cf-room.com', 'https://cf-room.com/information')
    $draftUrls = @([regex]::Matches($content, 'https://cf-room\.com[^"''\s<\\]*') | ForEach-Object { ($_.Value -split '#')[0].TrimEnd('/') } | Sort-Object -Unique)
    $unknown = @()
...
#     Counts href= links to cf-room.com pages (JSON-LD @id/url values have no href=, images excluded).
$bodyLinkUrls = @([regex]::Matches($content, 'href="(https://cf-room\.com/[^"]*)"') | ForEach-Object { ($_.Groups[1].Value -split '#')[0].TrimEnd('/') } | Where-Object { $_ -notmatch '/wp-content/' })
