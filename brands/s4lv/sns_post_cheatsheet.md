...
| Threads（@cfrms4lv・自動キュー） | **通常1日2〜3本**（2026-09-25オーナー確認・下記「Threads 運用値」） | 07:30／12:00／21:30（使う枠だけ） | キュー投入は `tools/queue_from_posts.py <file> "M/D " …`。時刻超過分は投入されないので当日午前分は繰り越す |

### Threads 運用値（役割ローテ・1行目・自己リプ・問い・見本の唯一の正＝`rules/threads_post_generation_rules.md`）

- 枠 07:30／12:00／21:30。**通常1日2〜3本**（2026-09-25オーナー確認。旧「1〜2本／世話できる日だけ3本」は撤回）・同じ文法の3本は出さない・間隔30〜60分（5〜10分連投禁止）
...
1a. **使う前に、台帳の各項目の「オーナー確認」行を必ず見る（2026-09-25新設）**：【使用停止】【保留】の項目は使わない（K9・A3・A4・A5・A7・A13・A16。最新は `x_neta_daicho.md` 冒頭「オーナー確認状況」）／「オーナー訂正」行がある項目は、訂正どおりに書く（K3・K4・K6・K10・K16・A1・A9）／K16は失敗談にしない（知識としてのみ）。**本文にも自己リプライにも、台帳にない体験談・行動（「私は最初〜を入れ忘れていた」等）を足さない**。新しい項目は、オーナーの「今もやっている」確認が取れるまで「候補」に置く
1a2. **「いまさら感」のある基礎の話・頻出ネタは避ける（2026-09-25）**：読んだ人が参考にでき、自分もやろうと思える中身（手順・基準・具体）にする。重複確認は `posts_x.txt` だけでなく**オーナーの実際のXタイムライン**でも見る（`posts_x.txt` は古い分を消すため、投稿済みの内容が漏れる）。K1は頻出のため当面使わない
