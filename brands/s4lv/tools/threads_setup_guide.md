...
> **⚠️ 2026-09-07改定済み**：この表は2026-08-23の初期設定記録。現在の投稿設計の唯一の正は `rules/threads_post_generation_rules.md`（本数・型ローテ・1行目・自己リプ・問い）。①本数＝**通常1日1〜2本／世話できる日だけ3本**（固定3本は廃止）。③**誘導リンクはフォロワー100まで全廃**（本文も自己リプもURLなし・noteはプロフィール欄のみ）。②③の下記記述は歴史的経緯として残置。

| # | 決定事項 | 確定内容（※①③は上の改定が優先） |
|---|---|---|
| ① | 投稿頻度 | ~~1日3本~~ → 通常1〜2本／世話できる日だけ3本（`threads_post_generation_rules.md`「本数・間隔」） |
