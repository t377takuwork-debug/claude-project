...
| タブ | 列 |
|---|---|
| Threads投稿キュー | 投稿日時 \| 本文 \| リプライ1本文 \| リプライ2本文 \| リプライ3本文 \| リプライ4本文 \| 型 \| FW \| ステータス \| 投稿ID \| リプライ1投稿ID \| リプライ2投稿ID \| リプライ3投稿ID \| リプライ4投稿ID \| **トピック**（2026-08-23、単発リプライ1本→連続スレッド最大4本連鎖へ拡張。2026-09-10、15列目に「トピック」を追加＝Threadsの topic_tag。メイン投稿にのみ付与・空可・`／トピック：XXX` を見出し末尾に書くとローカルツールが自動で列Oへ入れる） |
...
| `push_threads_queue.py <csv>` | CSV→投稿キュー投入。過去時刻スキップ・重複日時スキップ・投入後の日時昇順自動ソート付き（CSV9列目＝トピック。2026-09-10追加） |
...
> **⚠️ 2026-09-07改定済み**：この表は2026-08-23の初期設定記録。現在の投稿設計の唯一の正は `rules/threads_post_generation_rules.md`（本数・型ローテ・1行目・自己リプ・問い）。①本数＝**通常1日2〜3本**（2026-09-25オーナー確認）。③**誘導リンクはフォロワー100まで全廃**（本文も自己リプもURLなし・noteはプロフィール欄のみ）。②③の下記記述は歴史的経緯として残置。

| # | 決定事項 | 確定内容（※①③は上の改定が優先） |
|---|---|---|
| ① | 投稿頻度 | 通常1日2〜3本（2026-09-25オーナー確認・`threads_post_generation_rules.md`「本数・間隔」） |
| ② | 投稿設計（型・曜日配分・Note誘導ローテーション） | `/notekaigi`は開かず軽量版で開始。「型・思想6割／AI実働記録4割」比率はThreadsにも適用（比率6:4の骨格は変更なし。文体・生成ルールの唯一の正は `rules/threads_post_generation_rules.md`＋詳細 `feedback_s4lv_threads_writing_style.md`） |
| ③ | 自己リプライ（Note誘導URL枠）の使い方 | ~~URLは自己リプライ欄・4週間未誘導の記事を優先~~ → フォロワー100までは誘導URL全廃。100超で再開時にローテーション復活 |
