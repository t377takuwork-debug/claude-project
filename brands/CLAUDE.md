...
| タスク | 必須参照ファイル |
|---|---|
| MBTICODE SNS投稿・リプライ・引用ポスト生成 | `mbticode_strategy.md` / `mbticode/sns_post_cheatsheet.md` |
| MBTICODE Note記事生成 | `writing/writing_core.md` / `writing/writing_expressions.md` / `writing/writing_headlines.md` / `writing/writing_note_structure.md` / `mbticode_strategy.md` / `mbticode_tone.md` |
| s4lv_ai 作業 | `s4lv/ai/profile.md` |
| s4lv_pro 作業 | `s4lv/pro/profile.md` |
...
| `mbticode/posts/` | MBTICODE SNS投稿ファイル（[開始日]_[終了日]_posts.txt 形式・コピペ即使用） |
...
| `/note-article [アカウント] [テーマ]` | Note記事をペルソナ→タイトル→構成→本文まで一貫生成 | 記事を作るたびに |
