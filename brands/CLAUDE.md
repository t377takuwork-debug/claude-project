...
1. **捏造禁止**：コンテンツ作成時は各アカウントの `personal_data.md` を参照し、実体験に基づく内容のみ使用する（s4lv系→`s4lv/shared/personal_data.md`、MBTICODE→`mbticode/personal_data.md`）
   - **MBTICODE例外（2026-07-30）**：MBTICODEに限り、`personal_data.md`は「該当する実体験があれば参照する」ための参照データベースであり、内容を網羅するホワイトリストではない。実体験にない一般的な恋愛パターン語・トレンド語・キャラクター類型語も、ルール3（性的描写・断定的統計・特定個人を傷つける表現の禁止）に反しない範囲で使用してよい。ただし一人称の体験口調（「自分も〜だから言える」等）で語る場合は、読者に「発信者の実体験」と誤認させない一般化された書き方にする。s4lvには適用しない
...
| `/note-advisor` | s4lv Note運用アドバイザー（相談／KPI転記／台帳書き戻し／notekaigiへのエスカレーション）。主参照は `s4lv/rules/s4lv_learnings.md`。s4lvのみ・他アカウントは雛形化待ち | テーマ・価格・導線・記事の判断を気軽に相談するとき、KPIスクショを渡すとき |
