...
(exit code always 0) — it does not care WHY the queue is running low (forgot
to run apply_pending_batch.py, the weekly cloud routine failed its quality
gate, etc.), only that it is. That makes it robust to any failure mode in the
Phase3 pipeline (2026-07-26notekaigi), not just "human forgot the weekly
download+apply step".
"""
import datetime
import json
...
def main():
    ...
    if latest is None:
        print("[REMINDER] 投稿キューに日時付きの行が1件もありません。週次バッチの受け取りを確認してください。")
        return
    ...
    if days_remaining < WARN_BELOW_DAYS:
        print(f"[REMINDER] 投稿キューの残りは約{days_remaining:.1f}日分です（最終予定: {latest}）。"
              f"週次自動バッチ（https://claude.ai/code/routines/trig_011xykqfMBrvkzvnmUtrmjBa）の"
              f"実行結果を確認し、_pending_batch.jsonをダウンロードしてapply_pending_batch.pyで反映してください。")
    else:
