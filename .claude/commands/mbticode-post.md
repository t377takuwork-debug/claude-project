...
6. `brands/mbticode/threads_insights_notes.md` — 直近の週次分析結論（Threadsバッチの型・FW配分を決める前に一番上のエントリを確認する。「まだ分析記録なし」の場合は無視してよい）
...
2. **Step 2**：`threads_insights_notes.md`の最新エントリに「反映」事項があれば、その日の事前設計テーブル（型・FW配分）に反映した上で、その日（またはそのバッチ対象期間）分のX投稿・Threads事前設計テーブルをチャットに出力してユーザーの確認を取る
