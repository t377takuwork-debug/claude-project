...
1. **Step 1：事前設計テーブルの提示**
   ...
   - **Xの型・思想枠／AI実働枠は `x_neta_daicho.md`（柱1 K項目／柱2 A項目）からネタを起こす**。投稿案に「元ネタ＝台帳 K◯／A◯」を明記。台帳の「事実」欄の範囲外（＝創作）を書かない。台帳「候補」欄のネタはユーザー確認前は使わない。**使う前に、台帳の各項目の「オーナー確認」行を見る**（【使用停止】【保留】は使わない／「オーナー訂正」は訂正どおりに書く／本文・自己リプライに台帳にない体験談を足さない。2026-09-25、K/A全件のオーナー確認で判明。詳細は `sns_post_cheatsheet.md` 生成前チェック 1a・1a2）
   - **意見・考え枠（任意・柱3・2026-09-22新設）**：`x_neta_daicho.md`「柱3：意見・考え」にネタがあれば使ってよい。投稿案に「元ネタ＝台帳 柱3-◯」＋「対応する自分の考え＝K◯／A◯等」を明記。柱3は意見自体は実体験不要だが、**裏付けの具体エピソードは禁止**（`brands/CLAUDE.md`「捏造禁止」s4lv例外）。柱3にネタが無ければこの枠は無理に使わない
   ...
   - **意見・考え枠（任意・柱3・2026-09-22新設）**：`x_neta_daicho.md`「柱3：意見・考え」にネタがあれば使ってよい。投稿案に「元ネタ＝台帳 柱3-◯」＋「対応する自分の考え＝K◯／A◯等」を明記。裏付けの具体エピソードは禁止（`brands/CLAUDE.md`s4lv例外）。柱3にネタが無ければこの枠は無理に使わない
   - **Threadsの生成は `rules/threads_post_generation_rules.md` を開いて従う（唯一の正）**。要点：①1投稿ごとに役割（失敗談／知見／現場メモ）を先に決め、同じ骨格を3本続けない ②1行目は指摘・職歴で開かない＝現場か具体事実。「〜してないですか？」型は週1本まで（`qa_post.py` `th-shitenai-opener`）。「10年」等は1行目に置かず2文目・週2本まで ③誘導リンク全廃（本文も自己リプもURLなし・`qa_post.py` `th-url` がERROR。**旧・週1本のNote誘導CTAは2026-09-07プレイブックで廃止**）④問いで閉じるのは失敗談の回だけ（1バッチ2本前後）。知見は言い切り・現場メモは短く終わる ⑤自己リプは毎回付ける（例外／内訳／自分の抜け／技術的な詰まり のどれか1つ・本文の結論を繰り返さない）。AI実働の自己リプを毎回「エラーも出ず動かない」型にしない ⑥本数は通常1〜2本／世話できる日だけ3本（**旧・週3本固定は廃止**）
...
3.5. **Step 3.5a：自己レビュー（X・必須・2026-09-06追加。2026-09-10にバッチ横並びチェックを追加）**
   ...
   - **バッチ横並び（先にやる）**：6本の1行目だけを縦に並べて読む → 同じ入り方（過去失敗→今は／お題→説明→観察オチ／断言で開く 等）が3本以上なら書き直す。締めだけを並べて読む → 「〜ました。」で終わる本が2本以上なら散らす。`feedback_s4lv_x_writing_style.md`「生成後自己チェック（AI臭さ・構造レベル）」G＝バッチ単調さ
   - **各本**：1行目で何の話か（ブログ記事／検索順位 等の話題）が分かるか・断言を読者の理解より先に出していないか（`feedback_s4lv_x_writing_style.md`「生成後自己チェック（読者の入口）」）／3幕構成・箇条書きの対称性・教訓オチ・文リズムの均一さ
   - **オーナーの好み（`rules/s4lv_learnings.md`）＋句読点・改行・記号（`rules/sns_kutouten_kaigyo_rules.md`・X/Threads共通）に照らす**：句読点・改行・記号の型が入っているか・分かりにくい語（カタカナ・像を結べない語）がないか・数字が誇張気味でないか・オーナーが使わない決まり文句がないか
   - **ファイル内にURLがないか**（誘導回でも「投稿時に貼る」の指示だけ）
...
5. **Step 5：sns-ai-reviewer 審査（必須・スキップ禁止・2026-09-07 `/notekaigi` で `/post-review` から差し替え）**
   ...
   - **審査は原則1巡**（2026-09-10・トークン削減。1回 約85〜115k）。2巡目に回すのは BLOCK か FIX 3件以上のときだけ。それ未満の FIX は機械的修正を適用して保存
   - **オーナーが本文を自分で書き直した回は再委譲しない**（オーナーが判定者）。qa_post.py だけ通してユーザーへ返す
   - **修正は該当行の Edit で行う**（ファイル全体の Write をしない。qa_gate フックが保存時に自動で走る）
...
8. **Step 8：完了報告**
   ...
   **コミット（2026-09-10追加・トークン/往復削減）**：往復ごとにコミットしない。**バッチがオーナー確定した時点で1回**（台帳の書き戻しと同じコミットに含めてよい）。「まだ直す」と言われている間は保存だけでコミットしない。
