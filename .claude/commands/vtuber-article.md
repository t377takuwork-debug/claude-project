# vtuber_log 記事生成システム（画像自動配置つき）

vtuber_log（VTuber Log ・https://vtuber-verse.com/）の記事を、構成→本文→画像選定・配置→WordPressアップロード→QAまで一気通貫で生成する。会議は行わない。確定済みのvtuber_logルールを直接適用し、ユーザーの承認ポイントのみで進む。

## 呼び出し方

```
/vtuber-article [記事スラッグ] [テーマ/対策キーワード]
```

例：`/vtuber-article hoshimachi-suisei-profile 星街すいせい プロフィール`

**前提**：`blogs/vtuber_log/assets/<記事スラッグ>/` に記事で使う画像を投入済みであること。未投入なら「assetsフォルダに画像を入れてください」と案内して待つ。

## 実行前の準備（毎回必ず読む）

- `blogs/vtuber_log/rules/article_pipeline.md` — 工程順の1枚地図（本スキルはこれの自動化）
- `blogs/vtuber_log/rules/image_placement.md` — 画像選定・alt・配置・アップロードの全ルール
- 工程ごとに `article_pipeline.md` の参照ファイル列に従い該当ルールを開く（headline_writing・design_components等）
- 完成形リファレンス：`blogs/vtuber_log/drafts/azusa-honami-profile.txt`（デザイン・出力形式・VTuber記事の言い回しの見本）
...
1. `powershell -ExecutionPolicy Bypass -File "blogs/vtuber_log/tools/wp_upload.ps1" -Slug <スラッグ> -DryRun` で対象一覧を提示
...
1. 本文を生成する（`copywriting_phrases.md`のPASONA構成・`copywriting_phrase_bank.md`の言い回し・事実の捏造禁止。人物情報は確認できた事実のみ使用し、推測は推測と明示する）
...
4. 内部リンクを設置する（`internal_links.md`：実在URLのみ・本文2〜6本・重複なし。同一人物の別切り口記事＝プロフィール⇔中の人⇔前世は最優先候補）
5. JSON-LDを生成し（`structured_data_prompt.md`）、`wordpress_output_rules.md`の形式で `drafts/<スラッグ>.txt` に保存する

## Step 5：QAと記録（パイプライン工程10〜12）

1. `powershell -ExecutionPolicy Bypass -File "blogs/vtuber_log/tools/qa_draft.ps1" "drafts/<スラッグ>.txt"` を実行し**ERROR 0件**を確認する（画像はチェック8・12でURL照合される）
...
- 人物情報（前世・中の人・プロフィール）は確認できた情報のみ使用（捏造絶対禁止）。不明な事項はWebSearchで確認し、確認できなければ「未公表」「不明」と書く
